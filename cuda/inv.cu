...
__global__void suma(int *vector_1, int *vector_2, int *verctor_resultado,int n){

	// kernel multibloque 
	// identificaador global de cada hilo en el bloque 
	int index =threadIdx.x + blockDim.x*blockIdx;
	...
	if(index<n)
	{
		...
		vector_resultado[index]=vectcor_1[index]+vector_2[index];	
	}
